...
The Terraform CLI installation instructions have changed due to  keyring changes. So we needed to refer to the latest install CLI  via Terraform Documentation and change the installation scripting.
...
Please check your Linux Distribution to see and change accordingly to distribution needs.
...
You can also use the auto-approve flag to skip the approve prompt e.g.
...
When attempting to run `terraform login` it will launch bash a wishing view to generate a token. However, it does not work as expected in Gitpod VsCode in the browser.
